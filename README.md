...
difficulty class (DC) to beat and then a bonifiers list, which can be either
flat or specified in the typical die format (`XdY`, where `X` is the number of
dice and `Y` is their value). The bonifiers can be negative.

This is an example on how to use the code to calculate the odds to beat a 18 DC
with a 3d4 + 1 modifier

```sh
python3 odds_to_beat.py
Using a d20
Input difficulty class: 18 
